...
const GRID = '1.8fr 0.85fr 0.75fr 0.85fr 0.6fr 0.85fr 0.9fr 0.85fr'
...
export function CustomerListView({ initialBand, initialState }: Props) {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" style={{ background: '#070714' }}>
      ...
      {/* ── Top control bar — full width, flush to sidebar ── */}
      <div className="flex-shrink-0 px-5 pt-5 pb-0" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search customers…" value={search}
              ...
              className="pl-7 pr-3 py-1.5 rounded-lg text-[11px] outline-none w-52 transition-all"
              style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.65)' }}
            />
          </div>
        </div>

        {/* ── State tabs across the top ── */}
        <div className="flex items-center gap-1">
          ...
          {HEALTH_BANDS.map((b) => (
            <button key={b.key} onClick={() => setBand(b.key)}
              ...
              style={band === b.key
                ? { color: b.key === 'all' ? 'white' : b.color, borderBottomColor: b.key === 'all' ? 'rgba(255,255,255,0.5)' : b.color, background: 'transparent' }
                : { color: 'rgba(255,255,255,0.3)', borderBottomColor: 'transparent', background: 'transparent' }
              }
            >
              {b.key !== 'all' && (
          ...
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setDropdown(!dropdown)}
              className="flex items-center gap-1.5 px-3 py-2 text-[11px] font-medium transition-all border-b-2"
              style={state !== 'all'
                ? { color: STATE_COLORS[state] ?? '#00d4ff', borderBottomColor: STATE_COLORS[state] ?? '#00d4ff', background: 'transparent' }
                : { color: 'rgba(255,255,255,0.3)', borderBottomColor: 'transparent', background: 'transparent' }
              }
            >
              {state !== 'all' && (
                <div className="w-1.5 h-1.5 rounded-full" style={{ background: STATE_COLORS[state] }} />
              )}
              {stateLabelSel}
              ...
              <div className="absolute top-full left-0 mt-1 w-48 rounded-xl overflow-hidden z-50"
                ...
                {STATE_OPTIONS.map((opt) => (
                  <button key={opt.key}
                    ...
                    {state === opt.key && <span className="text-[#00d4ff] text-[9px]">✓</span>}
                    {opt.label}
                  </button>
                ))}
              </div>
          ...
          {loading ? (
            ...
            return (
              <div key={c.customer_id}
                className="grid py-3 cursor-pointer transition-all"
                style={{
