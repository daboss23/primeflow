...
export function Sidebar() {
  ...
  return (
    <aside className="w-52 flex-shrink-0 flex flex-col border-r border-white/[0.06] bg-[#070714]">
      <div className="px-4 pt-4 pb-3 border-b border-white/[0.06] flex items-center justify-center bg-[#070714]">
        <LogoPulse width={162} variant="sidebar" />
      </div>
      <nav className="flex-1 px-3 py-4 space-y-0.5">
        {NAV.map(({ href, label, icon }) => {
          const active = pathname === href || (href !== '/' && pathname.startsWith(href))
          return (
            <Link key={href} href={href} className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] font-medium transition-all ${active ? 'bg-white/[0.07] text-white' : 'text-white/40 hover:text-white/75 hover:bg-white/[0.04]'}`}>
