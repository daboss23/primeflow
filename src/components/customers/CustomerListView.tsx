...
const BANDS: { key: HealthBand | 'all'; label: string; dot?: string; glow?: string }[] = [
  { key: 'all',    label: 'All' },
  { key: 'red',    label: 'Critical', dot: '#ff4060', glow: 'rgba(255,64,96,0.5)'  },
  { key: 'yellow', label: 'Watch',    dot: '#ffaa00', glow: 'rgba(255,170,0,0.5)'  },
  { key: 'green',  label: 'Healthy',  dot: '#00e676', glow: 'rgba(0,230,118,0.5)'  },
]
...
function getActionCfg(state: string, spend: number): { label: string; color: string; bg: string; border: string } {
  const map: Record<string, { label: string; color: string; bg: string; border: string }> = {
    repeat_at_risk:      { label: 'Personal Outreach', color: '#a78bfa', bg: 'rgba(167,139,250,0.08)', border: 'rgba(167,139,250,0.22)' },
    dormant_buyer:       { label: 'Win-Back',           color: '#f59e0b', bg: 'rgba(245,158,11,0.08)',  border: 'rgba(245,158,11,0.22)'  },
    abandoned_cart:      { label: 'Cart Recovery',      color: '#ff6b6b', bg: 'rgba(255,107,107,0.08)', border: 'rgba(255,107,107,0.22)' },
    failed_payment:      { label: 'Payment Fix',        color: '#ff8c00', bg: 'rgba(255,140,0,0.08)',   border: 'rgba(255,140,0,0.22)'   },
    replenishment:       { label: 'Replenish Now',      color: '#00d4ff', bg: 'rgba(0,212,255,0.08)',   border: 'rgba(0,212,255,0.22)'   },
    engaged_unconverted: { label: 'Convert Now',        color: '#8b5cf6', bg: 'rgba(139,92,246,0.08)',  border: 'rgba(139,92,246,0.22)'  },
  }
  if (map[state]) return map[state]
  return spend > 3000
    ? { label: 'VIP Upgrade',    color: '#00e676', bg: 'rgba(0,230,118,0.08)',  border: 'rgba(0,230,118,0.22)'  }
    : { label: 'Loyalty Reward', color: '#00d4ff', bg: 'rgba(0,212,255,0.06)', border: 'rgba(0,212,255,0.18)' }
}

// ─── Health Cell ─────────────────────────────────────────────────────────────

function HealthCell({ score, band }: { score: number; band: HealthBand }) {
  const color = band === 'red' ? '#ff4060' : band === 'yellow' ? '#ffaa00' : '#00e676'
  const label = band === 'red' ? 'Critical' : band === 'yellow' ? 'Fair' : 'Good'
  return (
    <div className="flex items-center gap-2.5">
      <div className="flex items-center gap-1.5 w-[58px] flex-shrink-0">
        <div className="w-[7px] h-[7px] rounded-full flex-shrink-0"
          style={{ background: color, boxShadow: `0 0 6px ${color}, 0 0 10px ${color}55` }} />
        <span className="text-[11px] font-medium" style={{ color }}>{label}</span>
      </div>
      <div className="w-12 h-[3px] rounded-full overflow-hidden flex-shrink-0"
        style={{ background: 'rgba(255,255,255,0.07)' }}>
        <div className="h-full rounded-full"
          style={{ width: `${Math.min(100, score)}%`, background: `linear-gradient(90deg, ${color}66, ${color})` }} />
      </div>
    </div>
  )
}
  ...
  return (
    <>
      <style>{`
        ...
        .ci-row:hover { background: rgba(255,255,255,0.02) !important; }
        .ci-row:hover .ci-arrow { opacity: 1 !important; transform: translateX(0) !important; }
        .ci-arrow { opacity: 0; transform: translateX(-4px); transition: all 0.15s; }
        .ci-select {
          appearance: none; -webkit-appearance: none;
          background-image: url("data:image/svg+xml,%3Csvg width='10' height='6' viewBox='0 0 10 6' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1l4 4 4-4' stroke='rgba(255,255,255,0.25)' stroke-width='1.4' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E");
      ...
      <div className="flex-1 overflow-y-auto h-full" style={{ background: '#070714' }}>
        <div className="max-w-[1400px] mx-auto px-8 py-9">
          ...
          <div className="flex items-start justify-between mb-8">
            <div>
              ...
              <p className="text-[12px] ml-[18px]" style={{ color: 'rgba(255,255,255,0.3)' }}>
                ...
                {criticalCount > 0 && (
                  <span style={{ color: '#ff4060' }}> · {criticalCount} critical</span>
                )}
                {watchCount > 0 && (
                  <span style={{ color: '#ffaa00' }}> · {watchCount} on watchlist</span>
                )}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 mb-7 flex-wrap">
            ...
            {/* Street-light band pills */}
            <div className="flex items-center gap-1 p-1 rounded-xl"
              style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
              {BANDS.map(({ key, label, dot, glow }) => {
                const active = band === key
                return (
                  <button
                    key={key}
                    onClick={() => setBand(key as HealthBand | 'all')}
                    className="flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-[11px] font-medium transition-all"
                    style={active ? {
                      background: dot ? `${dot}14` : 'rgba(255,255,255,0.08)',
                      color: dot ?? 'rgba(255,255,255,0.9)',
                      border: `1px solid ${dot ? `${dot}30` : 'rgba(255,255,255,0.15)'}`,
                    } : {
                    ...
                    {dot && (
                      <span className="w-2 h-2 rounded-full flex-shrink-0" style={{
                        background: dot,
                        boxShadow: active ? `0 0 7px ${glow}, 0 0 12px ${glow}` : `0 0 3px ${dot}66`,
            ...
            <select
              ...
              className="ci-select px-3 py-2 rounded-xl text-[11px] font-medium outline-none transition-all cursor-pointer"
              style={{
            ...
            <div className="relative ml-auto">
              ...
              <input
                ...
                className="pl-8 pr-4 py-2 rounded-xl text-[11px] outline-none w-60"
                style={{
          ...
          <div className="rounded-2xl overflow-hidden"
            ...
            <div className="grid px-6 py-3.5"
              style={{
                gridTemplateColumns: '2fr 1.2fr 1.5fr 0.9fr 0.9fr 1.1fr 1.2fr 1.2fr',
                background: 'rgba(255,255,255,0.02)',
                borderBottom: '1px solid rgba(255,255,255,0.05)',
              }}>
              {['Customer', 'State', 'Health', 'Health Score', 'Opp. Score', 'Total Spend', 'Last Purchase', 'Action'].map((h) => (
                <span key={h} className="text-[10px] font-semibold tracking-[0.14em] uppercase"
                  style={{ color: 'rgba(255,255,255,0.2)' }}>
                  {h}
                </span>
              ))}
            ...
            {loading ? (
              ...
              <div className="py-20 text-center text-[13px]" style={{ color: 'rgba(255,255,255,0.18)' }}>
                  ...
                  <div
                    ...
                    {isUrgent && (
                      <div className="absolute left-0 top-3 bottom-3 w-[2px] rounded-r"
                        style={{ background: '#ff4060', boxShadow: '0 0 8px rgba(255,64,96,0.5)' }} />
                    )}

                    {/* Customer */}
                    <div className="flex items-center gap-3 pr-4">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-bold flex-shrink-0"
                        style={{ background: `${hColor}12`, color: hColor, border: `1px solid ${hColor}22` }}>
                        {ini}
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5">
                          <span className="text-[13px] font-semibold truncate"
                            style={{ color: 'rgba(255,255,255,0.88)' }}>{name}</span>
                          {isVIP && (
                    ...
                    <div className="flex items-center">
                      <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-semibold"
                        style={{
                          ...
                          border: `1px solid ${stateCfg.color}25`,
                        }}>
                        {stateCfg.label}
                      </span>
                    </div>
                    ...
                    {/* Health */}
                    <div className="flex items-center">
                      <HealthCell score={c.health_score} band={c.health_band} />
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-semibold whitespace-nowrap"
                        style={{
                      ...
                      <svg className="ci-arrow" width="10" height="10" fill="none" viewBox="0 0 16 16"
                        style={{ color: 'rgba(255,255,255,0.2)' }}>
...
path = os.path.expanduser("~/primeflow 2/src/components/customers/CustomerListView.tsx")
with open(path, "w") as f:
    f.write(content)

print("✅ Done — CustomerListView.tsx written successfully")
